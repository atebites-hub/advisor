...
printf '%s\n' "$doctor" | jq -e '
  .host == "zcode" and .strict == false and .code == "runtime_attestation_required" and
  .profileValid == true and .advisor.model == "zai/advisor" and .grunt.model == "zai/grunt" and
  .checks.hooks.trustObservable == false and .checks.hooks.trusted == false
' >/dev/null || fail "ZCode doctor did not report configured attestation-required status"
